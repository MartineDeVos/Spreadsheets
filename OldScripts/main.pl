...
%siblings(Cell,Sibling,SibLabel):-
%	edesign(Edesign),
%	row(Cell,Row),
%	col(Cell,Col),
%	(   row(Sibling,Row)
%	;   col(Sibling,Col) %same (row OR column)
%	),
%	sheet_label(Cell,SheetLabel), % AND same sheet
%	sheet_label(Sibling,SheetLabel),
%	rdf(Sibling,skos:prefLabel,literal(lang(_,SibLabel)),Edesign).

%agro_siblings(Cell, Label,List):-
%	cell_label_agro_match(Cell, Label, _Concept, _AgroLabel),
%	findall(SibLabel,siblings(Cell,_,SibLabel),List).
